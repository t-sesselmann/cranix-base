...
then
	...
	echo "INSERT INTO Devices VALUES(4,1,1,NULL,'printserver','${NEXTIP}',NULL,'','',0,0,'','','',0);" | msql CRX
	/usr/bin/systemctl restart cranix-api.service 
	sed -i "s/172.28.0.4/${NEXTIP}/" /etc/sysconfig/cranix
	/usr/sbin/crx_add_host.sh printserver ${NEXTIP}
