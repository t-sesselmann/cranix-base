...
for CLASS in $( /usr/sbin/oss_api_text.sh GET groups/text/byType/class )
do
        mkdir -p -m 750 /home/classes/$CLASS
        chown root:teachers /home/classes/$CLASS
done

for USER in  $( /usr/sbin/oss_api.sh GET users/uidsByRole/students )
do
        home=$( /usr/sbin/oss_get_home.sh ${USER} )
        ...
        for CLASS in $( /usr/sbin/oss_api_text.sh GET users/text/${USER}/classes )
        do
